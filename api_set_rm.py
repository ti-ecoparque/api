...
def processar_e_enviar_api_externa(num_rm, df_itens_rm, token_autenticado):
    ...
    faz os mapeamentos, cria a Requisição mãe e insere os itens filhos na API Externa.
    """
    ...
    cod_solicitacao = str(primeira_linha.get("cod_solicitacao_mega"))
    
    # Captura a data de entrega original e limpa espaços residuais
    ...
    try:
        ...
        st.write(f"✅ **Requisição Gerada no Sistema Destino!** ID: `{req_id}` | Nº Sequencial: `{num_sequencial}`")
        
    except Exception as e:
        return {"sucesso": False, "mensagens": f"❌ Falha de conexão ao criar requisição mãe: {e}"}
    ...
    # Criamos uma barra de progresso visual interna para o operador acompanhar os posts
    barra_itens = st.progress(0)
    total_linhas = len(df_itens_rm)
    
    for idx, (_, linha_item) in enumerate(df_itens_rm.iterrows()):
        # Captura o ID Externo (t_id) vindo do PROCV da tabela api_materiais
    ...
    if total_itens_inseridos > 0 and itens_com_falha == 0:
        ...
        return {
            "sucesso": True,
            "mensagens": f"⚠️ Integração Parcial: {total_itens_inseridos} itens enviados com sucesso, mas {itens_com_falha} falharam."
        }
    else:
        return {
            "sucesso": False,
